...
CREATE INDEX IF NOT EXISTS ix_air_booking_provider_ref
    ON air_cargo_bookings(tenant_id, provider, provider_reference);
CREATE INDEX IF NOT EXISTS ix_air_booking_status
    ON air_cargo_bookings(tenant_id, status, updated_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS ux_air_booking_operation_key
    ON air_cargo_bookings(tenant_id, last_operation_key)
...
CREATE INDEX IF NOT EXISTS ix_shipment_eta_history_lookup
    ON shipment_eta_history(tenant_id, shipment_id, observed_at DESC);
...
CREATE INDEX IF NOT EXISTS ix_airline_dead_letters_queue
    ON airline_integration_dead_letters(tenant_id, status, last_attempt_at DESC);

CREATE INDEX IF NOT EXISTS ix_integration_attempts_provider_status
    ON integration_attempts(tenant_id, provider, operation, status, created_at DESC);

DO $$ DECLARE t text; BEGIN
  FOREACH t IN ARRAY ARRAY['shipment_eta_history','airline_integration_dead_letters'] LOOP
...
CREATE TABLE IF NOT EXISTS airline_integration_alerts (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    resolved_at TIMESTAMPTZ,
    UNIQUE(tenant_id,alert_type,subject_ref,status)
);
CREATE INDEX IF NOT EXISTS ix_airline_integration_alerts_queue
    ON airline_integration_alerts(tenant_id,status,created_at DESC);

ALTER TABLE airline_integration_alerts ENABLE ROW LEVEL SECURITY;
ALTER TABLE airline_integration_alerts FORCE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation_airline_integration_alerts ON airline_integration_alerts
USING (tenant_id = NULLIF(current_setting('app.current_tenant', true), '')::uuid)
WITH CHECK (tenant_id = NULLIF(current_setting('app.current_tenant', true), '')::uuid);

CREATE UNIQUE INDEX IF NOT EXISTS ux_air_booking_idempotency
    ON air_cargo_bookings(tenant_id,idempotency_key);
