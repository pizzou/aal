...
public class AirCargoBooking {
 ...
 @Column(name="last_operation_key") private String lastOperationKey;
 @Column(name="last_provider_operation") private String lastProviderOperation;
 ...
 public void setServiceLevel(String serviceLevel){this.serviceLevel=serviceLevel==null?null:serviceLevel.trim();this.updatedAt=Instant.now();}
 public void operation(String key,String operation){this.lastOperationKey=key;this.lastProviderOperation=operation;this.updatedAt=Instant.now();}
 public void amend(BigDecimal weight,String flightNumber,Instant departure,Instant arrival,String serviceLevel,String providerReference,String raw){if(weight!=null)this.requestedWeightKg=weight; if(flightNumber!=null&&!flightNumber.isBlank())this.flightNumber=flightNumber; if(departure!=null)this.departureTime=departure; this.arrivalTime=arrival; this.serviceLevel=serviceLevel; if(providerReference!=null&&!providerReference.isBlank())this.providerReference=providerReference; this.rawResponse=raw; this.status="AMENDED"; this.updatedAt=Instant.now();}
 public void cancel(String reason,String raw){this.status="CANCELLED";this.cancellationReason=reason;this.cancelledAt=Instant.now();this.rawResponse=raw;this.updatedAt=Instant.now();}
